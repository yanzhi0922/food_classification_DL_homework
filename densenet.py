...
# 定义模型
class desnet_Classifier(nn.Module):
    def __init__(self):
        super(desnet_Classifier, self).__init__()
        # 使用 torchvision 自带的 resnet18
        self.densenet = torch.hub.load('pytorch/vision:v0.6.0', 'densenet121', pretrained=True)
        # 修改最后的 fully connected layer 使得输出是 5
        self.densenet.classifier = nn.Linear(1024, 5)
...
for epoch in range(num_epoch):
    ...
    with torch.no_grad():
        ...
        if (val_acc / val_set.__len__() > best_val_acc):
            ...
            torch.save(model, "model_densenet.pth")  # 保存最佳模型

# 保存模型, 以便之后测试
# torch.save(model, "model_densenet.pth")
print("Training complicated\n")

# 开始测试
print("Testing")
model = torch.load("model_densenet.pth")  # 加载模型
model.eval()
prediction = []
with torch.no_grad():
...
# 将结果写入 csv 档
with open("predict_densenet.csv", 'w') as f:
    f.write('Id,Category\n')
